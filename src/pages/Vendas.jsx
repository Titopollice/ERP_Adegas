...
const Vendas = () => {
  ...
  const handleAddItem = () => {
    if (produto && quantidade > 0) {
      ...
      toast.warn('Por favor, selecione um produto e uma quantidade válida.');
    }
  };
  ...
  const handleFinalizeSale = async () => {
    if (vendas.length === 0) {
      toast.warn('Adicione pelo menos um item à venda antes de finalizar.');
      return;
    }
  ...
  const aplicarDescontoFinal = () => {
    const novoDesconto = prompt("Digite o valor do desconto final:");
    if (novoDesconto !== null) {
      const descontoNumerico = parseFloat(novoDesconto);
      if (!isNaN(descontoNumerico) && descontoNumerico >= 0) {
        setDescontoFinal(descontoNumerico);
        const novoTotal = calcularValorTotal(vendas, descontoNumerico);
        setTotalVenda(novoTotal);
        toast.success(`Desconto de R$ ${descontoNumerico.toFixed(2)} aplicado com sucesso!`);
      } else {
